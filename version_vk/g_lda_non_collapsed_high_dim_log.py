...
class gLDA_nonCollapsed_highDim_log():
    """
    Non-Collapsed gLDA model for high dimensional data. To handle overflow issue, sample z in log space.
    """
    ...
    def sample_theta(self,i):
        for doc in range(self.num_doc):
            ...
            values, zks = np.unique(z_doc,return_counts=True) 
            zks_new = np.zeros((self.num_topic, ))
            for topic in range(self.num_topic):
                if topic in values:
                    zks_new[topic]= zks[np.argwhere(values==topic).item()]
                else:
                    zks_new[topic] = 0   
            self.thetas[i,doc,:] = sp.dirichlet.rvs(self.alpha + zks_new, size=1)
