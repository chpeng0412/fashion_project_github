...
class Gauss_LDA():
    def __init__(self, iters,  embeddings_file):
        #load data
        self.data_path = "../../data/"
        self.result_path = "../../results/collapsed_results/"
        self.simul_data= np.load(self.data_path+embeddings_file, allow_pickle=True).item()
        ...
        for topic in range(self.num_topic-1):
            print("true_prop:", np.sum(self.simul_data['z']==topic)/self.total_words)
        
        #set up priors
        self.alpha = self.simul_data['alpha']
        self.mu_0 = self.simul_data['mu_0']
        self.nu = self.simul_data['total_words']
    ...
    def sampling(self):
        ...
        for i in range(1, self.iters -1):
            ...
            ndk = self.ndk_func(i)
            theta_hat = ndk/np.sum(ndk, axis = 1).reshape(self.num_doc, 1)
            topic_prop = np.sum(theta_hat, axis =0)/np.sum(theta_hat)
...
if __name__ == '__main__':
    iters = 20
    data_file = 'simul_50_k_50_doc_1300_kappa_5.npy'
    instance= Gauss_LDA(iters,data_file) 
    instance.sampling()
                
    empri_prop = np.zeros((instance.iters,instance.num_topic)).astype('float32')
    for iter_ in range(instance.iters):
        for topic in range(instance.num_topic):
            empri_prop[iter_, topic] = np.sum(instance.zs[iter_, :,:] ==topic)/instance.total_words
    
    #do the time testing on high dimension
    
    for topic in range(instance.num_topic):
        fig, ax = plt.subplots()
        ax.plot(range(instance.iters), empri_prop[:,topic], label = "Topic Type " + str(topic) )
        leg = ax.legend()
        plt.title('Trajectory of Topic Types')
        plt.xlabel('Iters')
        plt.ylabel('Proportion')
        plt.savefig(instance.result_path+'dim_'+str(instance.num_embed) +'_k_'+str(instance.num_topic)+'_doc_'+str(instance.num_doc)+'_kappa_'+str(instance.kappa)+'_topic_types_'+str(topic))
        
    for topic in range(instance.num_topic):
        fig, ax = plt.subplots()
        ax.plot(range(instance.iters-1), instance.Ls_first[:instance.iters-1,topic, 0, 0], label = "Topic Type " + str(topic) )
        leg = ax.legend()
        plt.title('Trajectory of First Dimension Standard Deviation')
        plt.xlabel('Iters')
        plt.ylabel('Standard Deviation')
        plt.savefig(instance.result_path+'dim_'+str(instance.num_embed) +'_k_'+str(instance.num_topic)+'_doc_'+str(instance.num_doc)+'_kappa_'+str(instance.kappa)+'_standard_deviation_topic_type_'+str(topic))
